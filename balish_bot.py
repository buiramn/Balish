...
def kb_edit_fields(item_id):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🖼 Фото",       callback_data=f"ef_photo_{item_id}")],
...
async def add_icon(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ Иконка сақталды.\n\n*Фото* жіберіңіз немесе өткізіп жіберу үшін /skip басыңыз:",
        parse_mode="Markdown", reply_markup=kb_cancel(),
    )
    return ADD_CAT
...
async def add_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        ctx.user_data["new"]["photo_file_id"] = photo.file_id
        ...
        # Save to GitHub images folder
        item_name = ctx.user_data["new"].get("name", "item").replace(" ", "_")
        filename = f"images/{item_name}_{photo.file_id[:8]}.jpg"
        url = f"{API}/repos/{GITHUB_REPO}/contents/{filename}"
        r = requests.put(url, headers=gh_headers(), json={
            "message": f"📸 Фото қосылды: {item_name}",
            "content": b64,
            "branch": GITHUB_BRANCH,
        })
        if r.status_code in (200, 201):
            ctx.user_data["new"]["photo"] = filename
            await update.message.reply_text("✅ Фото жүктелді!\n\n*Санатты* таңдаңыз:", parse_mode="Markdown", reply_markup=kb_cats())
        else:
            await update.message.reply_text("⚠️ Фото жүктелмеді, өткізілді.\n\n*Санатты* таңдаңыз:", parse_mode="Markdown", reply_markup=kb_cats())
    return ADD_CAT

async def add_skip_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("*Санатты* таңдаңыз:", parse_mode="Markdown", reply_markup=kb_cats())
    return ADD_CAT
...
async def cb_edit_field(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "photo":
        await update.callback_query.edit_message_text(
            "📸 Жаңа фото жіберіңіз:", reply_markup=kb_cancel()
        )
        return EDIT_PHOTO
...
async def edit_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        img_bytes = bio.read()
        b64 = base64.b64encode(img_bytes).decode()
        await update.message.reply_text("⏳ GitHub-қа жүктелуде...")
        try:
            data, sha = github_load()
            item = next((i for i in data["menu"] if i["id"] == item_id), None)
            item_name = item["name"].replace(" ", "_") if item else "item"
            filename = f"images/{item_name}_{photo.file_id[:8]}.jpg"
            url = f"{API}/repos/{GITHUB_REPO}/contents/{filename}"
            r = requests.put(url, headers=gh_headers(), json={
                "message": f"📸 Фото жаңартылды: {item_name}",
                "content": b64,
                "branch": GITHUB_BRANCH,
            })
            if r.status_code in (200, 201):
                item["photo"] = filename
                github_save(data, sha, f"📸 {item_name} фотосы жаңартылды")
                await update.message.reply_text(
                    f"✅ Фото жаңартылды! Сайт жаңарады.", reply_markup=kb_back()
                )
            else:
                await update.message.reply_text(f"❌ Қате: {r.status_code}", reply_markup=kb_main())
        except Exception as e:
            await update.message.reply_text(f"❌ Қате: {e}", reply_markup=kb_main())
    else:
        await update.message.reply_text("⚠️ Фото жіберіңіз:", reply_markup=kb_cancel())
        return EDIT_PHOTO
    ctx.user_data.clear()
    return STATE_MAIN
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            ADD_CAT:   [
                ...
                CommandHandler("skip", add_skip_photo),
                CallbackQueryHandler(add_cat,  pattern="^cat_"),
                CallbackQueryHandler(cb_cancel, pattern="^cancel$"),
            ],
            EDIT_PHOTO: [
